...
typedef struct {
    ...
    uint32_t bytes_contig;    // 从 ISN 起连续确认的“字节数”（累计 ACK 指向位置）
                ...
                if (fid > 0) {
                    ...
                    if (tcp_payload_len >= sizeof(mini_hdr_t)) {
                        ...
                        if (tcp_payload_len >= sizeof(mini_hdr_t) + seg_len) {
                            ...
                            if (!st->inited) {
                                st->inited       = true;
                                st->client_isn   = seq - seg_idx * MSS_BYTES; // 反推 ISN
                                st->base_seg     = 0;
                                st->bytes_contig = 0;
                                for (int k=0; k<MAX_WINDOW_LEN; ++k) {
                                    st->win[k].have = false; st->win[k].len = 0;
                                }
                            }
