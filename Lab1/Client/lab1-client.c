...
/* === 统计：每条 flow 的开始/结束时间 & 时钟频率 === */
static uint64_t start_tsc[64], end_tsc[64];
static double   hz;

/* === ACK 32位回绕累加器（64位累计字节，支持>4GB流）=== */
static uint32_t last_ack32[64];
static uint64_t ack_bytes64[64];

static struct rte_ether_addr dst_mac = {{0xec,0xb1,0xd7,0x85,0x2a,0xf3}};  /* 按你环境改 */
...
lcore_main()
{
    ...
    for (int fid=1; fid<=flow_num; ++fid){
        ...
        /* 统计起点 */
        start_tsc[fid]   = rte_rdtsc();

        /* 回绕累加器初始化 */
        last_ack32[fid]  = client_isn[fid];
        ack_bytes64[fid] = 0;
    }
    ...
    while (active_flows > 0) {
        ...
        for (int fid=1; fid<=flow_num; ++fid){
            ...
            while (in_flight[fid] < MAX_WINDOW_LEN && next_seg[fid] < total_segs[fid]) {
                ...
                if (seg_idx == total_segs[fid]-1) {
                    ...
                    uint64_t sent_bytes = (uint64_t)seg_idx * (uint64_t)DATA_PER_SEG;
                    uint64_t remain = total_bytes[fid] - sent_bytes;
        ...
        for (uint16_t i=0;i<nb_rx;i++){
            ...
            if (fid>0 && fid<=flow_num) {
                uint32_t cur  = ack_num;
                uint32_t prev = last_ack32[fid];
                uint32_t d32;
                if (cur >= prev) {
                    d32 = cur - prev;
                } else {
                // wrap-around correction: e.g., prev near 4.2G, cur small (~100M)
                    d32 = (0xFFFFFFFFu - prev) + 1u + cur;
                }

                ack_bytes64[fid] += (uint64_t)d32;
                last_ack32[fid]   = cur;

                
                /* 32位ACK差值累计到64位，天然处理回绕 */
                // uint32_t cur  = ack_num;
                // uint32_t prev = last_ack32[fid];
                // uint32_t d32  = cur - prev;
                // ack_bytes64[fid] += (uint64_t)d32;
                // last_ack32[fid]   = cur;

                const uint64_t MINI = (uint64_t)sizeof(mini_hdr_t);
                uint64_t goal_bytes = (uint64_t)total_bytes[fid] + (uint64_t)total_segs[fid] * MINI;

                /* 已完成就直接忽略后续ACK（保护 active_flows 只减一次） */
                if (base_seg[fid] >= total_segs[fid]) { rte_pktmbuf_free(rx[i]); continue; }

                /* 字节级完成判定：ack 到达数据 + 所有 mini_hdr */
                if (ack_bytes64[fid] >= goal_bytes) {
                    in_flight[fid] = 0;
                    base_seg[fid]  = total_segs[fid];

                    /* 单流 latency / throughput */
                    end_tsc[fid] = rte_rdtsc();
                    double sec = (end_tsc[fid] - start_tsc[fid]) / hz;
                    double ms  = sec * 1000.0;
                    double gbps = ((double)total_bytes[fid] * 8.0) / sec / 1e9;

                    printf("[flow %d] done: bytes=%llu segs=%u (ack_bytes=%llu)  latency=%.3f ms  throughput=%.3f Gbps\n",
                        fid,
                        (unsigned long long)total_bytes[fid],
                        total_segs[fid],
                        (unsigned long long)ack_bytes64[fid],
                        ms, gbps);

                    active_flows--;
                } else {
                    /* 段级推进窗口（便于控制 in_flight） */
                    uint64_t ack_segs64 = ack_bytes64[fid] / (uint64_t)MSS_BYTES;
                    if (ack_segs64 > base_seg[fid]) {
                        uint32_t newly = (uint32_t)(ack_segs64 - base_seg[fid]);
                        base_seg[fid]  = (uint32_t)ack_segs64;
                ...
                /* 调试打印*/
                /*
                printf("[cli] fid=%d ack_num=%u (bytes=%llu goal=%llu) -> base=%u in_flight=%u\n",
                fid, ack_num,
                (unsigned long long)ack_bytes64[fid],
                (unsigned long long)goal_bytes,
                base_seg[fid], in_flight[fid]);
                */
            }
    ...
    /* Summary：总持续时间（并发测试）与平均带宽 */
    uint64_t t0 = start_tsc[1], t1 = 0;
    for (int fid=1; fid<=flow_num; ++fid) if (end_tsc[fid] > t1) t1 = end_tsc[fid];
    double tot_sec = (t1 - t0) / hz;

    double total_gbps   = ((double)flow_num * (double)total_bytes[1] * 8.0) / tot_sec / 1e9;
    double average_gbps = total_gbps / (double)flow_num;

    printf("[summary] flows=%d size=%lluB  total=%.3f Gbps  avg_per_flow=%.3f Gbps  duration=%.3f ms\n",
        flow_num,
        (unsigned long long)total_bytes[1],
        total_gbps, average_gbps, tot_sec*1000);
...
int main(int argc, char *argv[])
{
    ...
    if (argc == 3) {
        ...
        //wyj
        flow_size = (uint64_t) atoll(argv[2]);
    ...
    /* === 统计：记录时钟频率，并设置高分辨率随机种子 === */
    hz = (double)rte_get_tsc_hz();
    ...
    uint16_t port = 1;
    rte_eth_promiscuous_disable(port);   // 可选
